...
// path. Keep the latter usable when ASK_CARIBECON_MODE is unset; it returns the older flat
// {answer, citations} shape rather than a ResearchResult.
function appendLegacyAssistantMessage(question, body) {
  const citations = body.citations ?? [];
  const citationRows = citations
    .map(c => {
      const label = `${c.country} · ${c.label}`;
  ...
  const wrap = appendMessage(
    'msg msg--assistant',
    `<p class="msg__headline">${esc(body.answer ?? 'No answer was returned.')}</p>` +
      (citationRows ? `<div class="msg__cites">${citationRows}</div>` : ''),
  );
  ...
  insertButton.addEventListener('click', () => insertLegacyAnswer(question, body, insertButton));
  wrap.appendChild(insertButton);
  scrollMessagesToEnd();
}

async function insertLegacyAnswer(question, body, triggerEl) {
  const rows = [
    ['CaribEcon research', ''],
    ['Question', question],
    ['Answer', body.answer ?? ''],
