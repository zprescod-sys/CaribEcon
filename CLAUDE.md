...
- **Add FDI and spending-vs-revenue views to the Data page (mirror the budget pie).** Design is locked (no new data needed): **FDI** = cross-country ranked horizontal bars from `getIndicatorAllCountries('fdi')` (already US$ mn, directly comparable), year-in-focus selector, optional derived %-of-GDP toggle (`fdi_usd / (nominal_gdp_local / fx_rate_usd) × 100`). **Spending-vs-revenue** = per-country (dropdown like the budget pie) grouped bars of `govt_revenue_total` vs `govt_total_expenditure` (local currency, primary national sources), gap shaded as fiscal balance; render "Unavailable" where a country lacks the primary series.
- Optionally revive the removed feeds via an RSS.app/RSSHub proxy (Dominica News Online, IMF publication feeds, Loop News) in `data/feeds.json`.
